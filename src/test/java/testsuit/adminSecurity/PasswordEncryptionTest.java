...
public class PasswordEncryptionTest extends BasePage {
	...
	public void add_Password_Encryption_TC_01() throws Exception {
		if(!environment.contains("ALT") & !environment.contains("DOT") & !environment.contains("ROW") & !environment.contains("MVEA") & !environment.contains("CONNECTGEN") & !environment.contains("LYDIAN") & !environment.contains("AKUOENERGY") & !environment.contains("ORION") & !environment.contains("ATWELL") & !environment.contains("AVANTUS") & !environment.contains("NEXTERA") & !environment.contains("PRC") & !environment.contains("ORSTED")) {
		log("TC01 : Add Password Encryption");
		navigateToApplication(appURL);
		map = ExcelUtils.getRowFromRowNumber(prop.getProperty(Excel.LOGIN_TEST_DATA), Excel.Login, environment);
		objLogin.login(map);
		String testcaseName = "AddPasswordEncryption" + clientName;
		...
		objPasswordEncryption.setNewEncryptionPassword(map, testcaseName);
			...
			log("Menu has been removed from the base clients", Status.SKIP);
		}
	}
}
