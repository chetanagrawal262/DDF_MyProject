...
public class DeleteContactPage extends BasePage{
	...
	By btnDelete = By.xpath("//input[contains(@id,'_PRJCONTACTS_radYALGridControl_ctl00_ctl04_gbcDeleteAlignment') and @type='image']");
 ...
 public void DeleteContact(String testcaseName) {
		 ...
		 try {
			 ClickonCancle();
				log("STEP 2: Delete popup should Close and contacts record should not delete", Status.PASS);
			} catch (Exception e) {
		 ...
		 try {
			 ClickonOk();
				...
				log("STEP 3:  Project Contact record does not deleted", Status.FAIL);
