...
public class CustomControl_HTMLFORMULAFieldPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_HTMLFORMULAFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By VerifyField = By.xpath("//th[contains(text(),'ColumnHeader1')]");
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
	  ...
	  Set<String> windowId = driver.getWindowHandles(); // get window id of current window
		...
		By FieldNameOnParcelInfo = By.xpath("//*[contains(@id,'_tblCustomFieldOptions')]//*[text()='ColumnHeader1']");
  ...
  try {
	...
	try {
		...
		log("STEP 6: user can able to select the value HTML FORMULA  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value HTML FORMULA  from field type dd ", Status.FAIL);
	...
	if (util.isElementPresent(VerifyField)) {
		   log("STEP 15 : user can able to see  column header 1, colunm header 2, column header 3, column header 4  on preview   ", Status.PASS);
		} else {
		   log("STEP 15: user cannot able to see  column header 1, colunm header 2, column header 3, column header 4  on preview ", Status.FAIL);
