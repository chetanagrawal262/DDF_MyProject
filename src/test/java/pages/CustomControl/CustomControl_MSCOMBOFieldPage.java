...
public class CustomControl_MSCOMBOFieldPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_MSCOMBOFieldPage(WebDriver driver) {
		this.driver = driver;
	...
	By FieldValidation = By.xpath("//span[contains(text(),'MSCOMBO')]");
	...
	if(!util.isElementVisible(FieldName)) {  
		...
		try {
			if(environment.contains("TRA")) {
				SelectFieldType("MSCombo");
			}else {
			SelectFieldType(map.get(Excel.FieldType));
			}
			log("STEP 6: user can able to select the value \"MSCOMBO\" from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"MSCOMBO\"  from field type dd ", Status.FAIL);
		...
		try {
			...
			log("STEP 9: user can able to enter value in drop down category field    ", Status.PASS);
		} catch (Exception e) {
			log("STEP 9: user cannot able to enter value in drop down category field  ", Status.FAIL);
			throw new RuntimeException("Failed in step 9");
		}
		
		try {
			AddDropdownValue(map.get(Excel.DefaultValue));
			log("STEP 10: user can able to select the value from default value dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 10: user cannot able to enter value in drop down category field ", Status.FAIL);
		...
		if (util.isElementPresent(FieldValidation)) {
			   log("STEP 17 : user can able to see custom  field of type 'MSCOMBO' on preview  ", Status.PASS);
			} else {
			   log("STEP 17: user cannot able to see custom field of type 'MSCOMBO'  on preview  ", Status.FAIL);
