...
public class CustomControl_RICHTEXTFieldPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_RICHTEXTFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By txtText = By.xpath("(//iframe[contains(@id,'contentIframe')])[1]");
    public void AddDescription(String value) {
    	util.waitUntilElementDisplay(txtText);
    	if (!commonFunction.checkNA(value)) 
			util.inputTextAndPressTab(txtText, value);
    }
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
	...
	try {
		...
		log("STEP 6: user can able to select the value \"RICHTEXT\" from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"RICHTEXT\"  from field type dd ", Status.FAIL);
	...
	try {
		...
		log("STEP 9: user can able to enter text in default value field    ", Status.PASS);
	} catch (Exception e) {
		log("STEP 9: user cannot able to enter value in default value field ", Status.FAIL);
	...
	try {
		...
		log("STEP 10: user can able to enter value in Height field   ", Status.PASS);
	} catch (Exception e) {
		log("STEP 10: user cannot able to enter value in Height field  ", Status.FAIL);
	...
	if (util.isElementPresent(txtText)) {
		   log("STEP 17 : user can able to see custom control field of type 'RICHTEXT' ON PREVIEW   ", Status.PASS);
		} else {
		   log("STEP 17: user cannot able to see custom control field of type 'RICHTEXT' ON PREVIEW ", Status.FAIL);
	      throw new RuntimeException("Faild in STEP 17 ");
	} 
	
	try {
		AddDescription(map.get(Excel.Description));
		log("STEP 18: user can able to enter text in the custom field  of type 'RICHTEXT'  ", Status.PASS);
	} catch (Exception e) {
		log("STEP 18: user cannot able to enter text in the custom field  of type 'RICHTEXT'   ", Status.FAIL);
