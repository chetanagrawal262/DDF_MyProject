...
public class CustomControl_SECTIONHEADERFieldPage extends BasePage{

	
	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_SECTIONHEADERFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By FiledValidation = By.xpath("//span[contains(text(),'SECTONHEADER') and @title='ToolTip']");
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
		...
		try {
			...
			log("STEP 6: user can able to select the value \"SECTONHEADER\" from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"SECTONHEADER\"  from field type dd ", Status.FAIL);
		...
		By FiledValidation = By.xpath("//span[contains(text(),'"+map.get(Excel.DisplayName)+"') and @title='ToolTip']");
		if (util.isElementPresent(FiledValidation)) {
			   log("STEP 15 : user can able to see custom control field of type 'SECTIONHEADER' ON PREVIEW  ", Status.PASS);
			} else {
			   log("STEP 15: user cannot able to see custom control field of type 'SECTIONHEADER' ON PREVIEW ", Status.FAIL);
