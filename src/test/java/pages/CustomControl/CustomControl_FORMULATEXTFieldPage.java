...
public class CustomControl_FORMULATEXTFieldPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_FORMULATEXTFieldPage(WebDriver driver) {
		this.driver = driver;
  ...
  public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
	...
	try {
		...
		log("STEP 6: user can able to select the value \"FORMULA-TEXT\"  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"FORMULA-TEXT\"  from field type dd ", Status.FAIL);
	...
	By Validation = By.xpath("//span[contains(text(),'"+map.get(Excel.DisplayName)+"')]");
	if (util.isElementPresent(Validation)) {
		   log("STEP 15 : user can able to see the added custom field of type 'FORMULA-TEXT' under preview    ", Status.PASS);
		} else {
		   log("STEP 15: user cannot see the added custom field of type 'FORMULA-TEXT' under preview    ", Status.FAIL);
