...
public class CustomControl_GEOGRIDFieldPage extends BasePage{
	
	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_GEOGRIDFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By Addbtn = By.xpath("(//a[contains(@id,'lnkAddNewRecord')])[2]");
    
    public void ClickOnAddnewRecord() {
		 util.waitUntilElementDisplay(Addbtn);
		    util.click(Addbtn);
		     util.dummyWait(2);
	}
    ...
    By GridConfiguration = By.xpath("(//span[contains(@id,'ManageGridID') and @title='Grid Configuration'])[2]");
	By GridCheckbox = By.xpath("//input[contains(@id,'DisplayDownloadExcel')]");
	By btnUpdateGrid = By.xpath("//input[contains(@id,'idManage_UPDATE')]");
	By btnRefresh = By.xpath("//img[contains(@id,'Img1') and @alt='refresh']");
	By btnAddRecord = By.xpath("(//a[contains(@id,'lnkAddNewRecord')])[2]");
	By ManageCustomFieldRow = By.xpath("//span[contains(@title,'Manage custom field(s)')]");
	By ManageCustomField = By.xpath("(//span[contains(@title,'Manage custom field(s)')])[2]");
    
	By AddRecord = By.xpath("//a[contains(@id,'lnkAddNewRecord')]");
	By txtCustomField = By.xpath("//textarea[contains(@id,'EditFormControl_customFields2')]");
	By btnInsertField = By.xpath("//input[contains(@id,'EditFormControl_btnInsert')]");
	
	public void ClickOnInsert() {
		util.waitUntilElementDisplay(btnInsertField);
		util.click(btnInsertField);
	}
    
    public void ClickOnGridConfiguration() {
		util.waitUntilElementDisplay(GridConfiguration);
		util.dummyWait(1);
		util.click(GridConfiguration);
		util.dummyWait(2);
...
//		driver.switchTo().window(newAdwinID1);
		...
		util.waitUntilElementDisplay(GridCheckbox);
		util.click(GridCheckbox);
		util.waitUntilElementDisplay(btnUpdateGrid);
		util.click(btnUpdateGrid);
		driver.close();
	...
	public void ClickOnManageCustomFieldRow() {
		util.waitUntilElementDisplay(ManageCustomFieldRow);
		util.click(ManageCustomFieldRow);
	}
	
	public void ClickOnManageCustomField() {
		util.waitUntilElementDisplay(ManageCustomField);
		util.click(ManageCustomField);
	}
...
public void AddCustomField(String value) {
	...
	util.dummyWait(2);
	if (!commonFunction.checkNA(value)) 
		util.inputTextAndPressTab(txtCustomField, value);
}
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
	...
	try {
		...
		log("STEP 6: user can able to select the value \"GEOGRID\"  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"GEOGRID\"  from field type dd ", Status.FAIL);
	...
	if (util.isElementPresent(GridConfiguration)) {
		   log("STEP 16 : user can able to see the added custom field of type 'GEOGRID' under preview   ", Status.PASS);
		} else {
		   log("STEP 16:  user cannot see the added custom field of type 'GEOGRID' under preview  ", Status.FAIL);
	}
	try {
		ClickOnSaveHeader();
	...
	if (util.isElementPresent(Message)) {
		   ...
		   log("STEP 20: Views have been published successfully. message does not display ", Status.FAIL);
   	} 
