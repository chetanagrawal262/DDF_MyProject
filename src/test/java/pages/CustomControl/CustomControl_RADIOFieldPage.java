...
public class CustomControl_RADIOFieldPage extends BasePage{
	...
	public CustomControl_RADIOFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By VerifyCheckbox = By.xpath("(//input[contains(@id,'idMngCustmAdnlFields_customFields2') and @type='radio'])[1]");
    ...
    public void ClickOnCheckboxField() {
    	util.waitUntilElementDisplay(VerifyCheckbox);
    	util.click(VerifyCheckbox);
    	util.dummyWait(1);
    }
    ...
    By  Checkbox = By.xpath("(//label[contains(text(),'Horizontal')])[1]");
//    By VerifyCheckbox = By.xpath("(//input[contains(@id,'idMngCustmAdnlFields_customFields2') and @type='checkbox'])[1]");
    ...
    By txtDecimalValue = By.xpath("//input[contains(@id,'editcc_rdnDecimal') and @type='text']");
    public void AddDecimalValue(String value) {
    	if (!commonFunction.checkNA(value)) 
			util.inputTextAndPressTab(txtDecimalValue, value);
    }
    ...
    By checkboxSearchable = By.xpath("//input[contains(@id,'_EditFormControl_editcc_chkSearchable')]");
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
		...
		By FieldName = By.xpath("//*[contains(@id,'idMngCustmAdnlFields_')]/td[text()='" +map.get(Excel.FieldName)+ "' and not(contains(@style,'display'))]");
		...
		try {
			...
			log("STEP 6: user can able to select the value \"RADIO\" from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"RADIO\"  from field type dd ", Status.FAIL);
			throw new RuntimeException("Failed in step 6");
		}
		
		try {
			AddDropdownCategory(map.get(Excel.DropdownCategory));
			log("STEP 7: user can able add dropdown category ", Status.PASS);
		} catch (Exception e) {
			log("STEP 7: user can not able add dropdown category ", Status.FAIL);
		...
		try {
			ClickOnCheckbox();
			util.click(checkboxSearchable);
			log("STEP 10: user can able to select the any of the radio button from the options Vertical, Horizontal, Horizontal label left, Horizontal label right            ", Status.PASS);
		} catch (Exception e) {
			log("STEP 10: user cannot able to select the any of the radio button from the options Vertical, Horizontal, Horizontal label left, Horizontal label right  ", Status.FAIL);
			throw new RuntimeException("Failed in step 10");
		}
		
		try {
			AddPrefix(map.get(Excel.Prefix));
			log("STEP 11: user can able to enter text in prefix field   ", Status.PASS);
		} catch (Exception e) {
		...
		try {
			...
			log("STEP 13: user can able to enter text in comments field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 13: user cannot able to enter text in comments field ", Status.FAIL);
			throw new RuntimeException("Failed in step 13");
		}
		
		try {
			AddToolTip(map.get(Excel.ToolTip));
			log("STEP 14: user can able to enter text in Tool/tip field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 14: user cannot able to enter text in Tool/tip field   ", Status.FAIL);
		...
		if (util.isElementPresent(VerifyCheckbox)) {
			   log("STEP 20 : user can able to see custom control field of type 'RADIO' ON PREVIEW  ", Status.PASS);
			} else {
			   log("STEP 20: user cannot able to see custom control field of type 'RADIO' ON PREVIEW  ", Status.FAIL);
		...
		try {
			...
			log("STEP 21:  user can able to select the checkbox for above added custom field of type 'checkbox' on preview ", Status.PASS);
		} catch (Exception e) {
			log("STEP 21: user cannot select the checkbox for above added custom field of type 'checkbox' on preview    ", Status.FAIL);
	...
	try {
		...
		util.dummyWait(10);
		if(testcaseName.contains("CustomControl_RADIO_")) {
			driver.navigate().refresh();
		}
		log("STEP 22: user can able to navigate Parcel details page  ", Status.PASS);
	} catch (Exception e) {
		log("STEP 22: user can not navigate to Parcel details page ", Status.FAIL);
		throw new RuntimeException("Failed in step 22");
	}
	     
		try {
			Assert.assertTrue(util.isElementVisible(FieldNameOnParcelInfo));
			log("STEP 23: Added custom control field is dispaly on Parcel information page", Status.PASS);
		} catch (AssertionError e) {
			log("STEP 23:  Added custom control field does not dispaly on Parcel information page", Status.FAIL);
