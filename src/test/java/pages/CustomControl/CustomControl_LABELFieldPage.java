...
public class CustomControl_LABELFieldPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_LABELFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By Validation = By.xpath("//span[contains(text(),'LABEL')]");
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
	...
	try {
		...
		log("STEP 6: user can able to select the value LABEL from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value LABEL  from field type dd ", Status.FAIL);
	...
	if (util.isElementPresent(Validation)) {
		   log("STEP 15 : user can able to see custom  field of type 'LABEL' on preview   ", Status.PASS);
		} else {
		   log("STEP 15: user cannot able to see custom  field of type 'LABEL' on preview  ", Status.FAIL);
