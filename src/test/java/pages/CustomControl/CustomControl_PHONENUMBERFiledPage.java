...
public class CustomControl_PHONENUMBERFiledPage extends BasePage{

	
	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_PHONENUMBERFiledPage(WebDriver driver) {
		this.driver = driver;
    ...
    By txtPhoneNumber = By.xpath("//input[contains(@id,'customFields2') and @value='(___) ___-____']");
    public void AddPhoneNumber(String value) {
    	if (!commonFunction.checkNA(value)) 
			util.inputTextAndPressTab(txtPhoneNumber, value);
    }
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
		...
		try {
			...
			log("STEP 6: user can able to select the value \"PHONE NUMBER\" from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"PHONE NUMBER\"  from field type dd ", Status.FAIL);
		...
		try {
			...
			log("STEP 11: user can able to enter text in comments field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 11: user cannot able to enter text in comments field ", Status.FAIL);
			throw new RuntimeException("Failed in step 11");
		}
		
		try {
			AddToolTip(map.get(Excel.ToolTip));
			log("STEP 12: user can able to enter text in Tool/tip field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 12: user cannot able to enter text in Tool/tip field   ", Status.FAIL);
		...
		if (util.isElementPresent(txtPhoneNumber)) {
			   log("STEP 15 : user can able to see the added custom field of type 'PHONE NUMBER' under preview", Status.PASS);
			} else {
			   log("STEP 15: user cannot able to see the added custom field of type 'PHONE NUMBER' under preview ", Status.FAIL);
		      throw new RuntimeException("Faild in STEP 15 ");
	   	} 
		
		try {
			AddPhoneNumber(map.get(Excel.PhoneNo));
			log("STEP 16: user can able to enter value in custom field of type 'NUMERIC' on preview  ", Status.PASS);
		} catch (Exception e) {
			log("STEP 16:  user cannot able to enter value in custom field of type 'NUMERIC' on preview  ", Status.FAIL);
	...
	try {
		...
		util.dummyWait(10);
		log("STEP 2:0 user can able to navigate Parcel details page  ", Status.PASS);
	} catch (Exception e) {
