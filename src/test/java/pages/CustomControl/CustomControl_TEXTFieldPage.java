...
public class CustomControl_TEXTFieldPage extends BasePage{
	...
	public CustomControl_TEXTFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By btnDelete = By.xpath("(//input[contains(@id,'gbcDeleteAlignment') and @title='Delete'])[last()]");
    ...
    public void ClickOnDeleteCHECKBOXButton() {
    	util.waitUntilElementDisplay(btnDelete);
    	util.click(btnDelete);
    	util.dummyWait(1);
    }
    
    By fileUpload = By.xpath("(//input[contains(@id,'customFields2') and @type='file'])[1]");
	public void uploadFile() {
		String filepath = System.getProperty("user.dir") + File.separator + "test.txt";
		driver.findElement(fileUpload).sendKeys(filepath);
	}
    public void ClickOnDeleteCOLLAPSIBLEPANELButton() {
    	util.waitUntilElementDisplay(btnDelete);
    	util.click(btnDelete);
    	util.dummyWait(1);
    }
    
    public void ClickOnDeleteCURRENCYButton() {
    	util.waitUntilElementDisplay(btnDelete);
    	util.click(btnDelete);
    	util.dummyWait(1);
    }
    
    public void ClickOnDeleteNUMERICButton() {
    	util.waitUntilElementDisplay(btnDelete);
    	util.click(btnDelete);
    	util.dummyWait(1);
    }
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
    ...
    try {
		...
		try {
			...
			log("STEP 6: user can able to select the value \"TEXT\" from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"TEXT\"  from field type dd ", Status.FAIL);
		...
		try {
			...
			FIELD_NAME=map.get(Excel.DisplayName);
		...
		try {
			...
			log("STEP 11: user can able to enter value in Max Length/Value field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 11: user cannot able to enter value in Max Length/Value field   ", Status.FAIL);
		...
		try {
			...
			log("STEP 12: user can able to enter text in default value field    ", Status.PASS);
		} catch (Exception e) {
			log("STEP 12: user cannot able to enter value in default value field ", Status.FAIL);
		...
		By FieldValidation = By.xpath("//span[contains(text(),'"+map.get(Excel.DisplayName)+"')]");
		if (util.isElementPresent(FieldValidation)) {
			   log("STEP 20 : user can able to see custom control field of type 'TEXT' ON PREVIEW  ", Status.PASS);
			} else {
			   log("STEP 20: user cannot able to see custom control field of type 'TEXT' ON PREVIEW ", Status.FAIL);
