...
public class CustomControl_COLLAPSIBLEPANELFieldPage extends BasePage{

	
	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_COLLAPSIBLEPANELFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By ExpandPanel = By.xpath("(//span[contains(@class,'rpExpandHandle')])[1]");
	...
	try {
		...
		log("STEP 6: user can able to select the value \"COLLAPSIBLEPANEL\"  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"COLLAPSIBLEPANEL\"  from field type dd ", Status.FAIL);
	...
	if (util.isElementPresent(ExpandPanel)) {
		   log("STEP 15 : user can able to see the added custom field of type 'COLLAPSIBLEPANEL' under preview ", Status.PASS);
		} else {
		   log("STEP 15: user cannot see the added custom field of type 'COLLAPSIBLEPANEL' under preview   ", Status.FAIL);
	      throw new RuntimeException("Faild in STEP 15 ");
	}
	
	try {
		ClickOnExpandPanel();
		log("STEP 16: user can able to click on the expand arrow on preview ", Status.PASS);
	} catch (Exception e) {
		log("STEP 16: user cannot able to click on the expand arrow on preview ", Status.FAIL);
