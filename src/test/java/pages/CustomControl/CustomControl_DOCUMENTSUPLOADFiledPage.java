...
public class CustomControl_DOCUMENTSUPLOADFiledPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_DOCUMENTSUPLOADFiledPage(WebDriver driver) {
		this.driver = driver;
    ...
    By DocButton=By.xpath("//input[@class='ruButton ruBrowse']");
    ...
    By fileUpload = By.xpath("(//input[contains(@id,'customFields2') and @type='file'])[1]");
		public void uploadFile() {
		 ...
		 By fileUpload2 = By.xpath("(//input[contains(@id,'customFields2') and @type='file'])[2]");
    ...
    public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
		...
		try {
			...
			log("STEP 6: user can able to select the value \"DOCUMENTSUPLOAD\"  from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"DOCUMENTSUPLOAD\"  from field type dd ", Status.FAIL);
		...
		try {
			...
			log("STEP 9: user can able to enter text in the drop down category field  ", Status.PASS);
		} catch (Exception e) {
			log("STEP 9: user can able to enter text in the drop down category field       ", Status.FAIL);
		...
		try {
			uploadFile();
			util.dummyWait(1);
			log("STEP 16: user can able to select the document ", Status.PASS);
		} catch (Exception e) {
			log("STEP 16: user cannot able to select the document     ", Status.FAIL);
		...
		try {
			ClickOnRemove();
			log("STEP 17: user can able to click on remove icon  ", Status.PASS);
		} catch (Exception e) {
			log("STEP 17: user cannot  able to click on remove icon   ", Status.FAIL);
