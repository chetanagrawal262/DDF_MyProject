...
public class CustomControl_DATEFieldPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_DATEFieldPage(WebDriver driver) {
		this.driver = driver;
    ...
    By Calender = By.xpath("//a[contains(@id,'popupButton') and @title='Open the calendar popup.']");
    By txtDate = By.xpath("//a[contains(text(),'25')]");
    public void ClickOnCalender() {
    	util.waitUntilElementDisplay(Calender);
    	util.click(Calender);
    }
    
    public void SelectDate() {
    	util.waitUntilElementDisplay(txtDate);
    	util.click(txtDate);
    }
	...
	try {
		...
		log("STEP 6: user can able to select the value \"DATE\"  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"DATE\"  from field type dd ", Status.FAIL);
...
util.dummyWait(1);
	...
	if (util.isElementPresent(Calender)) {
		   log("STEP 15: user can able to see the added custom field of type 'DATE' under preview ", Status.PASS);
		} else {
		   log("STEP 15: user cannot see the added custom field of type 'DATE' under preview  ", Status.FAIL);
	      throw new RuntimeException("Faild in STEP 15 ");
	} 
	
	try {
		ClickOnCalender();
		log("STEP 16: user can able to see calender icon", Status.PASS);
	} catch (Exception e) {
		log("STEP 16: user cannot see calender icon   ", Status.FAIL);
		throw new RuntimeException("Failed in step 16");
	}
	
	try {
		SelectDate();
		log("STEP 17: selected date dispalys under field ", Status.PASS);
	} catch (Exception e) {
		log("STEP 17: selected date does not dispalys under field     ", Status.FAIL);
