...
public class CustomControl_CHECKBOXPage extends BasePage{
	
	TestUtil util;
	CommonFunction commonFunction;
	

	public CustomControl_CHECKBOXPage(WebDriver driver) {
		this.driver = driver;
	...
	try {
		...
		log("STEP 6: user can able to select the value \"CHECKBOX\"  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"CHECKBOX\"  from field type dd ", Status.FAIL);
	...
	try {
		ClickOnCheckbox();
		log("STEP 9:   user  can able to select radio button for option  ", Status.PASS);
	} catch (Exception e) {
		log("STEP 9: user cannot able to to select radio button for option ", Status.FAIL);
	...
	if (util.isElementPresent(VerifyCheckbox)) {
		   log("STEP 16 :  user can able to see the added custom field of type 'checkox' under preview ", Status.PASS);
		} else {
		   log("STEP 16: user cannot see the added custom field of type 'checkox' under preview", Status.FAIL);
	...
	try {
		...
		log("STEP 17:  user can able to select the checkbox for above added custom field of type 'checkbox' on preview ", Status.PASS);
	} catch (Exception e) {
		log("STEP 17: user cannot select the checkbox for above added custom field of type 'checkbox' on preview    ", Status.FAIL);
