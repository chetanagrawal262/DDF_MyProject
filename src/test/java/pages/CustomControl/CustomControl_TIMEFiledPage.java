...
public class CustomControl_TIMEFiledPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_TIMEFiledPage(WebDriver driver) {
		this.driver = driver;
    ...
    By Clock = By.xpath("//a[@title='Open the time view popup.']");
    By SelectTime = By.xpath("//a[contains(text(),'3:00 AM')]");
    public void ClickOnClock() {
    	util.waitUntilElementDisplay(Clock);
    	util.click(Clock);
	...
	try {
		...
		log("STEP 6: user can able to select the value \"TIME\"  from field type dd ", Status.PASS);
	} catch (Exception e) {
		log("STEP 6: user can not able to select the value \"TIME\"  from field type dd ", Status.FAIL);
	...
	if (util.isElementPresent(Clock)) {
		   log("STEP 15 : user can able to see the added custom field of type 'TIME' under preview ", Status.PASS);
		} else {
		   log("STEP 15: user cannot see the added custom field of type 'TIME' under preview  ", Status.FAIL);
	...
	try {
		ClickOnClock();
		log("STEP 16: selected time displays under field   ", Status.PASS);
	} catch (Exception e) {
		log("STEP 16: selected time does not displays under field ", Status.FAIL);
