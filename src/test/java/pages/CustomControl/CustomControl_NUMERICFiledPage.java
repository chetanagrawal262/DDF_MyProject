...
public class CustomControl_NUMERICFiledPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;

	public CustomControl_NUMERICFiledPage(WebDriver driver) {
		this.driver = driver;
    ...
    By FieldValidation = By.xpath("//span[contains(text(),'NUMERIC')]");
...
public void CustomControl(Map<String, String> map, String testcaseName) throws InterruptedException {
	...
	if(!util.isElementVisible(FieldName)) {
		...
		try {
			...
			log("STEP 6: user can able to select the value \"NUMERIC\" from field type dd ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user can not able to select the value \"NUMERIC\"  from field type dd ", Status.FAIL);
		...
		try {
			...
			log("STEP 9: user can able to enter value in Max length value field   ", Status.PASS);
		} catch (Exception e) {
			log("STEP 9: user cannot able to enter value in Max length value field  ", Status.FAIL);
			throw new RuntimeException("Failed in step 9");
		}
		
		try {
			AddMinValue(map.get(Excel.MinValue));
			log("STEP 10:  user can able to enter value in Min.numeric field       ", Status.PASS);
		} catch (Exception e) {
			log("STEP 10: user cannot able to enter value in Min.numeric field   ", Status.FAIL);
			throw new RuntimeException("Failed in step 10");
		}
		
		try {
			AddMaxValue(map.get(Excel.MaxValue));
			log("STEP 11:  user can able to enter value in Max.numeric field       ", Status.PASS);
		} catch (Exception e) {
			log("STEP 11: user cannot able to enter value in Max.numeric field   ", Status.FAIL);
			throw new RuntimeException("Failed in step 11");
		}
		
		try {
			AddDecimalValue(map.get(Excel.DecimalValue));
			log("STEP 12: user can able to enter value in # of decimal places  ", Status.PASS);
		} catch (Exception e) {
			log("STEP 12: user cannot able to enter value in # of decimal places ", Status.FAIL);
			throw new RuntimeException("Failed in step 12");
		}
		
		try {
			AddPrefix(map.get(Excel.Prefix));
			log("STEP 13: user can able to enter text in prefix field   ", Status.PASS);
		} catch (Exception e) {
		...
		try {
			...
			log("STEP 15: user can able to enter text in comments field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 15: user cannot able to enter text in comments field ", Status.FAIL);
			throw new RuntimeException("Failed in step 15");
		}
		
		try {
			AddToolTip(map.get(Excel.ToolTip));
			log("STEP 16: user can able to enter text in Tool/tip field ", Status.PASS);
		} catch (Exception e) {
			log("STEP 16: user cannot able to enter text in Tool/tip field   ", Status.FAIL);
		...
		if (util.isElementPresent(FieldValidation)) {
			   log("STEP 19 : user can able to see the added custom field of type 'NUMERIC' under preview    ", Status.PASS);
			} else {
			   log("STEP 19: user cannot able to see the added custom field of type 'NUMERIC' under preview   ", Status.FAIL);
