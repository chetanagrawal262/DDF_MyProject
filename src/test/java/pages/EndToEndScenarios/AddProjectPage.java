...
//				.findElement(By.xpath("//*[contains(@id,'RadPageView')]/..//*[contains(text(),'Project #')]/..//strong")).getText();
...
//System.out.println(getStrProjectCode);
System.out.println(getStrProjectName);
//System.out.println(getProjectCodeSummary);
...
//			log("STEP 22: Project Code Matched !!!", Status.PASS);
//		} else {
//			System.out.println("Project Code not Matched");
//			log("STEP 22: Project Code not Matched", Status.FAIL);
//		}
	...
	By firstType = By.xpath("//*[contains(@id,'OBJPROJECTINFO_yalProjType_radYALDropDownList_DropDown')]/..//li[1]");
	By firstTypePopup = By.xpath("//*[contains(@id,'prjInfo_yalProjType_radYALDropDownList_DropDown')]/..//li[1]");
	...
	By firstArea = By.xpath("//*[contains(@id,'OBJPROJECTINFO_yalPrjArea_radYALDropDownList_DropDown')]/..//li[1]");
	By firstAreaPopup = By.xpath("//*[contains(@id,'prjInfo_yalPrjArea_radYALDropDownList_DropDown')]/..//li[1]");
	...
	By firstStatus = By.xpath("//*[contains(@id,'OBJPROJECTINFO_PROJECT_STATUS_ID_radYALDropDownList_DropDown')]/..//li[1]");
	By firstStatusPopup = By.xpath("//*[contains(@id,'prjInfo_PROJECT_STATUS_ID_radYALDropDownList_DropDown')]/..//li[1]");
	...
	public void addProjectInformation(Map<String, String> map,String testCaseName) throws InterruptedException {
		...
		if(util.isElementPresent(frmFull)){
			...
			if(environment.contains("TALENENERGY")) {
				...
				try {
					//setAreaPopUp(map.get(Excel.TalenEnergyArea));
				...
				try {
					setCostCenterPopUp(map.get(Excel.TalenEnergyCostCenter));
					log("STEP 8: User can enter value in the workday cost center field", Status.PASS);
				} catch (Exception e) {
					log("STEP 8: User cannot enter value in the workday cost center field", Status.FAIL);
					throw new RuntimeException("Failed in step 8");
				}
				
				try {
					setBudgetItemPopUp(map.get(Excel.TalenEnergyBudgetItem));
					log("STEP 9: User can enter value in the workday Budget Item field", Status.PASS);
				} catch (Exception e) {
					log("STEP 9: User cannot enter value in the workday Budget Item field", Status.FAIL);
			...
			try {
				util.waitUntilElementDisplay(drpClientPopUp);
				util.click(drpClientPopUp);	
				util.click(firstAreaPopup);
				//setClientPopUp(map.get(Excel.Client));
				log("STEP 8: User can select Client value from the DD", Status.PASS);
			} catch (Exception e) {
				log(" STEP 8: User cannot select Client value from the DD", Status.FAIL);
				throw new RuntimeException("Failed in step 8");
			}
			...
			try {
				...
				log("STEP 16: User can click on Insert and Close Button", Status.PASS);
			} catch (Exception e) {
				log("STEP 16: User cannot click on Insert and Close Button ", Status.FAIL);
		...
		if(environment.contains("TALENENERGY")) {
			...
			try {
				...
				//setArea(map.get(Excel.TalenEnergyArea));
				log("STEP 7: User can select Area value from the DD", Status.PASS);
			} catch (Exception e) {
				log("STEP 7: User cannot select Area value from the DD ", Status.FAIL);
				throw new RuntimeException("Failed in step 7");
			...
			try {
				setCostCenter(map.get(Excel.TalenEnergyCostCenter));
				log("STEP 8: User can enter value in the workday cost center field", Status.PASS);
			} catch (Exception e) {
				log("STEP 8: User cannot enter value in the workday cost center field", Status.FAIL);
				throw new RuntimeException("Failed in step 8");
			...
			try {
				setBudgetItem(map.get(Excel.TalenEnergyBudgetItem));
				log("STEP 9: User can enter value in the workday Budget Item field", Status.PASS);
			} catch (Exception e) {
				log("STEP 9: User cannot enter value in the workday Budget Item field", Status.FAIL);
				throw new RuntimeException("Failed in step 9");
		...
		try {
			...
			log("STEP 10 : Added Client value displays in the field", Status.PASS);
		} catch (Exception e) {
			log("STEP 10 :  User cannot Select Client value from the DD ", Status.FAIL);
			throw new RuntimeException("Failed in step 10");
		}
		if(testCaseName.contains("ROW") || testCaseName.contains("TRA")) {
			try {
				setProjectStatusROW();
				log( "STEP 11 : Added Project Status value displays in the field", Status.PASS);
			} catch (Exception e) {
				log("STEP 11 : User cannot select Project Status value from the DD ", Status.FAIL);
				throw new RuntimeException("Failed in step 11");
			}
		}
		else {
...
//				setProjectStatus(map.get(Excel.ProjectStatus));
		...
		try {
			...
			log("STEP 12 : Added TemporaryROW value displays in the field", Status.PASS);
		} catch (Exception e) {
			log("STEP 12 : User cannot select TemporaryROW value from the DD ", Status.FAIL);
			throw new RuntimeException("Failed in step 12");
		}
			}
		...
		try {
			...
			log("STEP 13 : Added PermanentROW value displays in the field", Status.PASS);
		} catch (Exception e) {
			log("STEP 13 : User cannot select PermanentROW value from the DD ", Status.FAIL);
			throw new RuntimeException("Failed in step 13");
		}
			}
			...
			if(util.isElementPresent(drpBoardResolution)) {
				try {
					...
					log("STEP 14 : User should be able to select the Board Resolution", Status.PASS);
				} catch (Exception e) {
					log("STEP 14 : User cannot Select board resolution value from the DD", Status.FAIL);
					throw new RuntimeException("Failed in step 14");
				}
					}
		...
		try {
			...
			log("STEP 15 : Added Unit value displays in the field", Status.PASS);
		} catch (Exception e) {
			log("STEP 15 : User cannot select value from the Unit DD ", Status.FAIL);
			throw new RuntimeException("Failed in step 15");
		}
			}
		}
		try {
			...
			log("STEP 16 : Added SAP Company Code value displays in the field", Status.PASS);
		} catch (Exception e) {
			log("STEP 16 : User cannot select value from the SAP Company code DD", Status.FAIL);
			throw new RuntimeException("Failed in step 16");
		}
		try {
			setSAPPayingEntity(map.get(Excel.PayingEntity));
			log("STEP 17 : Added SAP Paying Entity value displays in the field", Status.PASS);
		} catch (Exception e) {
			log("STEP 17 : User cannot select value from the SAP Paying Entity DD", Status.FAIL);
			throw new RuntimeException("Failed in step 17");
		}
		try {
			setSAPProjectEntity(map.get(Excel.ProjectEntity));
			log("STEP 18 : Added SAP Project Entity value displays in the field", Status.PASS);
		} catch (Exception e) { 
			log("STEP 18 : User cannot select value from the SAP Project Entity DD ", Status.FAIL);
		...
		if(util.isElementVisible(drpWBS)) {
			try {
				setWBS(map.get("WBS"));
				log("STEP 18 : Added WBS value displays in the field", Status.PASS);
			} catch (Exception e) { 
				log("STEP 18 : User cannot select value from the WBS DD ", Status.FAIL);
				throw new RuntimeException("Failed in step 18");
			}
			}
		...
		try {
			...
			log("STEP 19: User can not click on save button ", Status.FAIL);
