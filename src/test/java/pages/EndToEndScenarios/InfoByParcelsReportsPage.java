...
public class InfoByParcelsReportsPage extends BasePage {
	...
	public void verifyReportData(String testcaseName) throws Exception {
		...
		ExtentTestManager.info("Name : " + ParcelInformation_AddNewParcelPage.MANUALLYADDED_PARCEL);
		...
		sa.assertTrue(util.isElementVisible(addedName, 30),"Name " + ParcelInformation_AddNewParcelPage.MANUALLYADDED_PARCEL + " is not added in Report");
	...
	public void viewSSRSReport(Map<String, String> map, String testcaseName) throws Exception {
		...
		try {
			for(String var: parceList) {
				count++;
				ExtentTestManager.info("Imported Parcel " +count+" : "+ var);
						By addedRecord = By.xpath("//*[text()='"+var+"']");
						WebElement element = driver.findElement(addedRecord);
						new Actions(driver).moveToElement(element).perform();
						sa.assertTrue(util.isElementVisible(addedRecord, 30), "Record "+var+" is not added in Report");
						if(count==39) {
							clickOnNextPage();
						}
				}
			log("STEP 5:  All Imported Parcels are visible in the Info By Parcels Report", Status.PASS);
		} catch (Exception e) {
			log("STEP 5:  All Imported Parcels does not d in the Info By Parcels Report", Status.FAIL);
