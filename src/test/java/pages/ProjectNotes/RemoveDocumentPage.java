...
public class RemoveDocumentPage extends BasePage{
	...
	By RemoveDocument = By.xpath("//span[contains(text(),'Remove Document')]");
	 ...
	 public void NavigateToParcelNotesAlt() {
	  ...
	  By ViewDocument = By.xpath("//tbody/tr[contains(@id,'_AGENTNOTES_radYALGridControl_ctl00__0')]/td[2]/..//a[contains(text(),'View Document')]");
	  public void RemoveDocument(String testcaseName) {
		  ...
		  if (testcaseName.contains("DOT") || testcaseName.contains("ROW") || testcaseName.contains("TRA")) {
  			log("Remove  Document not applicable for DOT and ROW ", Status.SKIP);
  		}
	  	   ...
	  	   try {
	  		 ...
	  		 ClickonRemoveDocument();
	  		   ...
	  		   util.reloadPage();
	  		   util.dummyWait(3);
	  		if(!util.isElementVisible(ViewDocument)) {
