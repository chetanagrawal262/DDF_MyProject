...
//				 f.exists(); // this will print true or false whether the file exists
	...
	public void uploadFile(String testcasename) {
		...
		if (testcasename.contains("DOT")) {
			if(testcasename.contains("DocumentImport")) {
			String filepath = System.getProperty("user.dir") + File.separator + ".\\ImportExcels//DOT-Document Import.xlsx";
			driver.findElement(fileUpload).sendKeys(filepath);
			}
			else {
				String filepath = System.getProperty("user.dir") + File.separator + ".\\ImportExcels//DOT-Consolidated Import Template.xlsx";
				driver.findElement(fileUpload).sendKeys(filepath);
			}
		}
		util.dummyWait(5);
	...
	public void uploadInvalidFile(String testcasename) {
		...
		if (testcasename.contains("DOT")) {
			...
			driver.findElement(fileUpload).sendKeys(filepath);
				...
				String filepath = System.getProperty("user.dir") + File.separator + ".\\ImportExcels//DOT-Invalid_Consolidated_Import_Template.xlsx";
				driver.findElement(fileUpload).sendKeys(filepath);
			}
		}
		util.dummyWait(5);
	...
	public void setWorkSheetTalenEnergy(String value) {
		util.waitUntilElementDisplay(drpWorkSheet);
		value = "'" + value + "'";
		util.inputTextAndPressTab(drpWorkSheet, value);
	}
	...
	public void setPagesize(){
		...
		System.out.println("");
		By pageSize50 = By.xpath("//*[contains(@id,'PageSizeComboBox_DropDown') ]/div/ul/li[3]");
	...
	public void invalidProjectDataImport(Map<String, String> map, String testcaseName) throws Exception {
		...
		util.dummyWait(10);
		try {
			Assert.assertTrue(util.isElementVisible(errorMessage, 20), "Error message not displayed");
