...
public class TitleFormPage extends BasePage {
	...
	public TitleFormPage(WebDriver driver) {
		this.driver = driver;
	...
	By createRoleDD = By.xpath("//span[text()='Title']/../..//input[contains(@id,'_cbCreateRoles') and @type='text']");
	By viewRoleDD = By.xpath("//span[text()='Title']/../..//input[contains(@id,'_cbViewRoles') and @type='text']");
	...
	public void TitleFormConfiguration(Map<String, String> map, String testcaseName)
			throws InterruptedException {
		
		try {
			navigateToTitle();
			log("STEP 1:  user can navigate to Title Tab", Status.PASS);
		} catch (Exception e) {
			log("STEP 1:  user cannot navigate to Title Tab", Status.FAIL);
		...
		By name = By.xpath("(//*[contains(text(),'"+map.get("FieldName")+"')]/..)[1]");
		
		if(!util.isElementVisible(name)) {
		try {
			ClickOnAddNewRecordLink();
		...
		try {
			By valueName = By.xpath("(//*[contains(text(),'"+map.get("FieldName")+"')]/..//textarea)[1]");
			util.inputTextAndPressTab(valueName, map.get("Approver Name"));
		...
		try {
			closeApprovalForm();
			log("STEP 35: user can close the popup", Status.PASS);
		} catch (AssertionError e) {
			log("STEP 35: user can not close the popup", Status.FAIL);
