...
public class ConfigureCustomerOutreachFormPage extends BasePage {
	...
	public ConfigureCustomerOutreachFormPage(WebDriver driver) {
		this.driver = driver;
	...
	By panelCOForm = By.xpath("//span[contains(text(),'Customer Outreach Forms')]");
	...
	public void COFormConfiguration(Map<String, String> map, String testcaseName)
			throws InterruptedException {
		if(testcaseName.contains("ROW") || testcaseName.contains("TRA")) {
    		try {
    			NavigateToTicketInformationRow();
    			SelectTicket("Working Tickets");
    				log("STEP 1: User Can Navigate to Ticket Information ", Status.PASS);
    			} catch (Exception e) {
    				log("STEP 1:  User Cannot Navigate to Ticket Information", Status.FAIL);
		...
		try {
			Assert.assertTrue(util.isElementVisible(panelCOForm));
			log("STEP 1: User can see Customer Outreach Form Panel", Status.PASS);
		} catch (Exception e) {
			log("STEP 1: User cannot see Customer Outreach Form Panel", Status.FAIL);
		}
		...
		try {
			By textNotificationRecipients = By.xpath("//table[contains(@id,'dWorkflowNotificationConfig_NOTIFICATIONCONFIGID_radYALGridControl')]//td[text()='Security Role: "+map.get("Reviewer")+"']");
		...
		try {
			if(!util.isElementVisible(btnAddNewRecord)) {
			SelectTicket("Working Tickets");
			}
			ClickOnAddNewRecord();
		...
		try {
			closeApprovalForm();
			log("STEP 53: user can close the popup", Status.PASS);
		} catch (AssertionError e) {
			log("STEP 53: user can not close the popup", Status.FAIL);
