...
public class RelocationformsPage extends BasePage{

	TestUtil util;
	CommonFunction commonFunction;
	public static String AGREEMENT_NUMBER;

	public RelocationformsPage(WebDriver driver) {
		this.driver = driver;
    ...
    public void Relocationforms(Map<String, String> map, String testcaseName) {
    	 try {
    		 ClickOnAddButton();
    	 ...
    	 try {
    		 ...
    		 try {
 	    	 ...
 	    	 try {
 	 			...
 	 			log("STEP 18:   User can click on submit for review button.  ", Status.PASS);
 	 		 } catch (Exception e) {
 	 			log("STEP 18:  User cannot click on submit for review button  ", Status.FAIL);
 	 			throw new RuntimeException("Failed in step 18");
 	 		 }
 	    	 
 	    	 try {  
     		 ClickOnStep1Link();
  			log("STEP 19: Step Name Link should be clicked successfully ", Status.PASS);
  		 } catch (Exception e) {
  			log("STEP 19:  Pop Up should not appeared   ", Status.FAIL);
     	 ...
     	 try {
  			...
  			log("STEP 21: User can enter Comment ", Status.PASS);
  		 } catch (Exception e) {
  			log("STEP 21: User cannot enter Comment  ", Status.FAIL);
     	 ...
     	 try {
     		 ...
     		 util.switchToIframe(FullFrm);
     		 util.dummyWait(1);
  			log("STEP 22: User can click on Approve button successfully  ", Status.PASS);
  		 } catch (Exception e) {
  			log("STEP 22: Pop Up should not appeared   ", Status.FAIL);
  			throw new RuntimeException("Failed in step 22");
  		 }
     	 
     	 try {
     		 ClickonRequesterLink();
  			log("STEP 23: User can click on requestor link   ", Status.PASS);
  		 } catch (Exception e) {
  			log("STEP 23: User cannot click on requestor link  ", Status.FAIL);
