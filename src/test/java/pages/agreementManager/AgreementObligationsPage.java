...
public class AgreementObligationsPage extends BasePage {
	...
	By firstType = By.xpath("(//*[contains(@id,'EditFormControl_TYPE_ID_')]/..//li[not(contains(text(),'Select One'))])[1]");
	public void enterObligationType() {
		util.click(obligationType);
		util.waitFor(100);
	...
	public void addAgreementObligations(Map<String, String> map, String testcaseName) {
		...
		try {
			...
			log("STEP 2: User cannot select value from the Type DD", Status.FAIL);
			throw new RuntimeException("Failed in step 2");
		}
		if(!environment.contains("CONNECTGEN") & !environment.contains("ORION") & !environment.contains("ATWELL")) {
		try {
			enterObligationPriority(map.get(Excel.Priority));
			log("STEP 3: Fill in Priority DD", Status.PASS);
		} catch (Exception e) {
			log("STEP 3: User cannot select value from the priority DD", Status.FAIL);
		...
		try {
			...
			log("STEP 4: User cannot select value from the Status DD", Status.FAIL);
		...
		try {
			...
			log("STEP 5: User cannot select value from the Frequency DD", Status.FAIL);
		...
		try {
			...
			log("STEP 6: User cannot enter value in the Date approved field", Status.FAIL);
		...
		try {
			...
			log("STEP 7: User cannot enter value in the Start Date field", Status.FAIL);
		...
		try {
			...
			log("STEP 8: User cannot enter value in the End Date field", Status.FAIL);
		...
		try {
			...
			log("STEP 9: User cannot select value from the Assigned To DD", Status.FAIL);
		...
		try {
			...
			log("STEP 10: User cannot enter value in the Last Completed Date", Status.FAIL);
		...
		try {
			...
			log("STEP 11: User cannot enter value in the Next Due Date", Status.FAIL);
		...
		try {
			...
			log("STEP 12: User cannot enter the Description", Status.FAIL);
		...
		try {
			...
			log("STEP 13: User cannot click on Insert", Status.FAIL);
	...
	public void editAgreementObligations(Map<String, String> map, String testcaseName) {
		try {
			clickOnEditButton();
			log("STEP 1: Click on the edit button on the Agreement Obligations grid", Status.PASS);
		} catch (Exception e) {
			log("STEP 1: User cannot click on the edit button on the Agreement Obligations grid", Status.FAIL);
			throw new RuntimeException("Failed in step 1");
		}
		...
		try {
			...
			log("STEP 2: User cannot update the values for some of the Fields", Status.FAIL);
			throw new RuntimeException("Failed in step 2");
		}
		...
		try {
			...
			log("STEP 3:Unable to Click on Update", Status.FAIL);
			throw new RuntimeException("Failed in step 3");
		}
	}
	...
	public void deleteAgreementObligations(String testcaseName) {
		try {
			...
			log("STEP 1: User cannot click on the Delete button for the record in the Agreement Obligations panel ", Status.FAIL);
			throw new RuntimeException("Failed in step 1");
		}
		...
		try {
			...
			log("STEP 2: User cannot click on the Cancel button in the Delete popup", Status.FAIL);
			throw new RuntimeException("Failed in step 2");
		}
		...
		try {
			...
			log("STEP 3: click OK button in the Delete popup", Status.PASS);
		} catch (Exception e) {
			log("STEP 3: User cannot click OK button in the Delete popup", Status.FAIL);
			throw new RuntimeException("Failed in step 3");
		}
	}
}
