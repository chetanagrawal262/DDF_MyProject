...
public class AddProjectPage extends BasePage {
	...
	public static String PROJECT_NAME, PROJECT_NUMBER;
	...
	By btnArchieve = By.xpath("//input[contains(@id,'_OBJPROJECTINFO_btnArchive') and contains(@onclick,'ArchiveProject')]");
...
//		}
		if (getProjectNameSummary.equals(getStrProjectName)) {
			...
			log("STEP 20: Project Name Matched !!!", Status.PASS);
		} else {
			System.out.println("Project Name not Matched");
			log("STEP 20: Project Name not Matched", Status.FAIL);
		}

	}

	public void setProjectNumber() {
		PROJECT_NUMBER = util.randomNumber();
		util.inputText(txtProjectNumber, PROJECT_NUMBER);
	}
	
	public void setProjectNumberPopUp() {
		PROJECT_NUMBER = util.randomNumber();
		util.inputText(txtProjectNumberPopUp,PROJECT_NUMBER);
	}
	...
	public void setWBSPopUp(String value) {
		if (!commonFunction.checkNA(value))
			util.inputTextAndPressTab(drpWBSPopUp, value);
	...
	public void addProjectInformation(Map<String, String> map,String testCaseName) throws InterruptedException {
		...
		if(util.isElementPresent(frmFull)){
			...
			try {
				...
				ExtentTestManager.info("Enter Project Name: " +map.get(Excel.ProjectName));
			...
			if(util.isElementVisible(drpWBSPopUp)) {
				try {
					setWBSPopUp(map.get("WBS"));
					log("STEP  Added WBS value displays in the field", Status.PASS);
				} catch (Exception e) { 
					log("STEP  User cannot select value from the WBS DD ", Status.FAIL);
					throw new RuntimeException("Failed to select wps on popup");
				}
				}
		...
		if(util.isElementVisible(drpArea)) {
		try {
...
//				setProjectStatus(map.get(Excel.ProjectStatus));
		...
		try {
			...
			if(util.isElementPresent(drpBoardResolution)) {
				try {
					...
					log("STEP 11 : User should be able to select the Board Resolution", Status.PASS);
				} catch (Exception e) {
					log("STEP 11 : User cannot Select board resolution value from the DD", Status.FAIL);
					throw new RuntimeException("Failed in step 11");
				}
					}
		...
		try {
			...
			log("STEP 12 : User cannot select value from the Unit DD ", Status.FAIL);
			throw new RuntimeException("Failed in step 12");
		}
			}
		}
		try {
			...
			log("STEP 13 : User cannot select value from the SAP Company code DD", Status.FAIL);
			throw new RuntimeException("Failed in step 13");
		}
		try {
			...
			log("STEP 14 : User cannot select value from the SAP Paying Entity DD", Status.FAIL);
			throw new RuntimeException("Failed in step 14");
		}
		try {
			...
			log("STEP 15 : User cannot select value from the SAP Project Entity DD ", Status.FAIL);
			throw new RuntimeException("Failed in step 15");
		}
		...
		try {
			...
			log("STEP 17: User can not click on save button ", Status.FAIL);
		...
		try {
			...
			log("STEP 18: The button text should be changed to UNARCHIVE", Status.PASS);
		} catch (Exception e) {
			log("STEP 18: Archive Button text or color does not change ", Status.FAIL);
			throw new RuntimeException("Failed in step 18 ");
		}
		util.dummyWait(5);
		try {
			commonFunction.clickOnSaveButton();
			log("STEP 19: Project is unarchive and does not display in Poject list DD", Status.PASS);
		} catch (Exception e) {
			log("STEP 19: Project still displays under the project list", Status.FAIL);
			throw new RuntimeException("Failed in step 19 ");
		}
		}
		}
		util.dummyWait(5);
		if(!environment.contains("DOMINION")) {
		verifySummaryALT();
		}
		...
		if(environment.contains("WOLFMIDSTREAM")) {
		try {
			By projectTitle = By.xpath("//div[@id='ctl00_ConPHLeftTop_PROJLST_ddt_ProjectList' and @title='"+PROJECT_NUMBER+"-"+PROJECT_NAME+"']");
			Assert.assertTrue(util.isElementPresent(projectTitle));
			log("STEP 21: User can see Project Title in 'Project#-ProjectName' format in project dropdown", Status.PASS);
		} catch (AssertionError e) {
			log("STEP 21: User cannot see Project Title in 'Project#-ProjectName' format in project dropdown", Status.FAIL);
		}
		}
	}
