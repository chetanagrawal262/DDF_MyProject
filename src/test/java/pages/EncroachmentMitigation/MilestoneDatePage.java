...
public class MilestoneDatePage extends BasePage {
	...
	By panelMilestoneDates = By.xpath("//span[contains(text(),'Milestone Dates')]");
    ...
    By txtComments = By.xpath("//textarea[contains(@id,'ENROACHKEYDATES_radYALGridControl_ctl00_ctl05_EditFormControl_tb_Comment')]");
    ...
    public void ExpandMilestoneDate() {
    	util.waitUntilElementDisplay(panelMilestoneDates);
		util.click(panelMilestoneDates);
	}
	...
	public void AddMilestoneDate(Map<String, String> map, String testcaseName) {
		...
		try {
			ExpandMilestoneDate();
			log("STEP 1: User can expand milestone dates panel ", Status.PASS);
		} catch (Exception e) {
			log("STEP 1:  User cannot expand milestone dates panel ", Status.FAIL);
