...
public class UserRoleTemplatePermissionPage extends BasePage {
	...
	private void uncheckecTemplatePermissionnOption(String value) {
		if (!commonFunction.checkNA(value)) {
			By checkedTemplatePermission = By.xpath("//*[text()='"+value+"']/../input[@checked='checked']");
			if(util.isElementVisible(checkedTemplatePermission)) {
			util.click(checkedTemplatePermission);
			}
		}
		
	}
	
	private void checkedTemplatePermissionnOption(String value) {
		if (!commonFunction.checkNA(value)) {
	...
	public void selectTemplatePermission(Map<String, String> map, String testcaseName) {
		...
		try {
			...
			log("STEP 3:  User can click on Template Permission", Status.PASS);
		} catch (Exception e) {
			log("STEP 3:  User can not click on Special Permission", Status.FAIL);
			throw new RuntimeException("Failed in step 3:  User can not click on Template Permission");
		}
		try {
			uncheckecTemplatePermissionnOption(map.get(Excel.TemplatePermissions));
			log("STEP 4:  User can uncheck special permission ", Status.PASS);
		} catch (Exception e) {
			log("STEP 4:  User can not uncheck special permission ", Status.FAIL);
		...
		try {
			checkedTemplatePermissionnOption(map.get(Excel.TemplatePermissions));
			log("STEP 6:  User can check special permission ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6:  User can not check special permission ", Status.FAIL);
			throw new RuntimeException("STEP 6:  User can not select TemplatePermissions option");
		}
		try {
			clickOnSavePermission();
			...
			throw new RuntimeException("Failed in step 7: User can not click on Save ");
		}
