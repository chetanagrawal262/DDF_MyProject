...
public class ManageUserAddNewUserPage extends BasePage {
	...
	By role = By.xpath("(//*[contains(@id,'EditFormControl_ROLES')]/..//input)[1]");
...
//		}
	...
	public void editNewUser(String value) {
		...
		if (util.isElementPresent(pagesize)) {
			...
			if (!util.isElementPresent(editNewUser)) {
				By btn = By.xpath(
	...
	public void updateNewUser(Map<String, String> map, String testcaseName) {
		try {
			...
			log("STEP 20:  User can refresh the record", Status.PASS);
		} catch (Exception e) {
			log("STEP 20:  User cannot refresh the record", Status.FAIL);
