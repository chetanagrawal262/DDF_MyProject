...
public class AddDocumentExternalLinkPage extends BasePage{
	...
	public static String DRAFT_FINAL;
	...
	By txtDescription = By.xpath("//input[contains(@id,'radReportConfigPanels_i0_i0_description')]");
    ...
    public void SelectDraftFinalExtLink(String value) {
    	if (!commonFunction.checkNA(value))
			util.inputTextAndPressTab(drpDraftFinalExternalLink, value);
    ...
    public void AddDocumentExternalLink(Map<String, String> map, String testcaseName) {
  		...
  		if(testcaseName.contains("ALT") || testcaseName.contains("ORSTED")||testcaseName.contains("ORION") || testcaseName.contains("ATWELL")) {
	    	 ...
	    	 try {
	    		 SelectDraftFinalExtLink(map.get("DraftFinal"));
	    	 ...
	    	 try {
	    		 util.dummyWait(1);
	    		 ClickOnAddALTButton();
	 			log("STEP 9: User can click on add button.", Status.PASS);
	 		 } catch (Exception e) {
	 			log("STEP 9: User can not click on add button.", Status.FAIL);
 	    	...
 	    	try {
 	    	 ...
 	    	 try {
 	    		 ClickOnAddButton();
 	 			log("STEP 7: User can click on add button.", Status.PASS);
 	 		 } catch (Exception e) {
 	 			log("STEP 7: User can not click on add button.", Status.FAIL);
	    	 ...
	    	 try {
	    		 Assert.assertTrue(util.isElementVisible(txtEffectiveDateExtLink));
	 			log("STEP 7:  User can see Effective Date field on External link   ", Status.PASS);
	 		 } catch (AssertionError e) {
	 			log("STEP 7: User cannot see Effective Date field on External link ", Status.FAIL);
	 		 }
