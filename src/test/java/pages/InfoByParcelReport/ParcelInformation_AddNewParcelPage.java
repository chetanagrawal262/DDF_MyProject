...
public class ParcelInformation_AddNewParcelPage extends BasePage
{
	...
	public static String MANUALLYADDED_PARCEL;
   ...
   // Step No 5
    ...
    By txtLeasedAcres = By.xpath("//*[text()='Leased Acres:']/..//input[@type='text']");	
    ...
    public void AddParcel(Map<String, String> map, String testcaseName) {
    	  try {
    	   ...
    	   addParcel();
    	   log("STEP 1: User can click on the add Parcel button  ", Status.PASS);
    	} catch (Exception e) {
		   log("STEP 1:  User cannot see the add Parcel button  ", Status.FAIL);
    	   ...
    	   if(util.isElementPresent(frmFullALT)) {
    		   ...
    		   try {
	    			...
	    			log("STEP 3:  Added value displays in Property Type field   ", Status.PASS);
	    		} catch (Exception e) {
	    			log("STEP 3: User  cannot select  value  from Property Type DD  ", Status.FAIL);
    		   ...
    		   if(util.isElementVisible(drpAcquisitionPriorityPopUp)) {
	        	try {
	    			...
	    			log("STEP 4:  Added value displays in Acquisition Priority field   ", Status.PASS);
	    		} catch (Exception e) {
	    			log("STEP 4: User  cannot select  value  from Acquisition Priority DD  ", Status.FAIL);
	    			throw new RuntimeException("Failed in step 4");
	    		}
    		   }
	            try {
	            	addParcelNoPopUp(map.get(Excel.ParcelNo));
	            	MANUALLYADDED_PARCEL=map.get(Excel.ParcelNo);
	    			log("STEP 5:  User can enter a value in Parcel Number field    ", Status.PASS);
	    		} catch (Exception e) {
	    			log("STEP 5:  User cannot add a value in Parcel Number field ", Status.FAIL);
    	    	    ...
    	    	    if(util.isElementPresent(drpStatePopUp)) {
    	            	try {
    	            	     selectStatePopUp(map.get(Excel.State));                                         
    	           		     log("STEP 6:  User can select value from the State DD  ", Status.PASS);
    	           	    } catch (Exception e) {
    	           		     log("STEP 6: User  cannot select  value  from the State DD  ", Status.FAIL);
    	           		     throw new RuntimeException("Failed in step 6");
    	           	    }
    	            	
    	            	try {
    	            	     selectCountyPopUp(map.get(Excel.County));                                         
    	           		     log("STEP 7:  User can select value from the County DD  ", Status.PASS);
    	           	    } catch (Exception e) {
    	           		     log("STEP 7: User  cannot select  value  from the County DD  ", Status.FAIL);
    	      ...
    	      try {
    				...
    				log("STEP 8: Added value displays in Title Status field   ", Status.PASS);
    			} catch (Exception e) {
    				log("STEP 8:  User  cannot select  value  from Title Status DD  ", Status.FAIL);
...
//        	selectPropertyType(map.get(Excel.PropertyType));
        ...
        try {
			...
			log("STEP 6:  user is able to select value from Project Package dd    ", Status.PASS);
		} catch (Exception e) {
			log("STEP 6: user not able to select value from  Project Package dd   ", Status.FAIL);
			throw new RuntimeException("Failed in step 6");
		}
...
//        		}
      ...
      if(util.isElementVisible(txtParcelAcres)) {
       ...
       try {
			...
			log("STEP 12: User can enter a value in Parcel Acres field     ", Status.PASS);
		} catch (Exception e) {
			log("STEP 12:  User cannot enter a value in Parcel Acres field      ", Status.FAIL);
      ...
      if(util.isElementVisible(txtGISAcres)) {
        try {
			...
			log("STEP 13: User can enter a value in GIS Acres field     ", Status.PASS);
		} catch (Exception e) {
			log("STEP 13:   User cannot enter a value in GIS Acres field     ", Status.FAIL);
      ...
      if(util.isElementVisible(txtLeasedAcres)) {
        try {
			...
			log("STEP 14: User can enter a value in Leased Acres field   ", Status.PASS);
		} catch (Exception e) {
			log("STEP 14:  User cannot enter a value in Leased Acres field   ", Status.FAIL);
        ...
        if(testcaseName.contains("ROW") || testcaseName.contains("TRA"))
      {
        ...
        try {
          	clickOnSave();
          	util.dummyWait(2);
    			log("STEP 15: Parcel is created sucessfully    ", Status.PASS);
    		} catch (Exception e) {
    			log("STEP 15:  Parcel is not created    ", Status.FAIL);
